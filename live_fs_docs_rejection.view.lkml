view: live_fs_docs_rejection {
  derived_table: {
    sql: select
      ...
      left join base_status sdvr on sdvr.order_id = bo.id and sdvr.status = 'Settlement Docs Rejected'
      left join base_customeruserprofile cup on cup.user_id = bo.user_id
      left join base_statushistory bsh on bsh.order_id = bo.id and bsh.status = 'Settlement Docs Rejected'
      left join base_statustransitionreasons bstr on bsh.transition_reason_enum = bstr.id
      where bo.status = 'Settlement Docs Rejected'
       ;;
  }
  ...
  dimension: sdv_rejected_time {
    type: string
    label: "SDV Rejected Time"
    sql: ${TABLE}.`SDV Rejected Time` ;;
  }
