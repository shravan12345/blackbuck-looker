view: base_order {
  ...
  measure: Acceptance_alert {
    type: number
    sql: CASE WHEN TIMESTAMPDIFF(hour,${OA.dt_updated_raw},${TAS.dt_updated_raw}) >  2 THEN 1 ELSE 0 END  ;;
  }
  measure: TAS_alert {
    type: number
    sql: CASE WHEN TIMESTAMPDIFF(hour,${TAS.dt_updated_raw},${ADV.dt_updated_raw}) >  6 THEN 1 ELSE 0 END  ;;
  }
  measure: ADV_alert {
    type: number
    sql: CASE WHEN TIMESTAMPDIFF(hour,${ADV.dt_updated_raw},${PP.dt_updated_raw}) >  1 THEN 1 ELSE 0 END  ;;
  }
  measure: PP_alert {
    type: number
    sql: CASE WHEN TIMESTAMPDIFF(minute,${PP.dt_updated_raw},${TDS.dt_updated_raw}) >  30 THEN 1 ELSE 0 END  ;;
  }
