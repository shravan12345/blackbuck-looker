view: latest_customer_rate {
  derived_table: {
    sql: SELECT ETA1.id,ETA1.rate,ETA1.customer_id,ETA1.from_city_id,ETA1.to_city_id , ETA1.truck_type_id,ETA1.last_updated_at,ETA1.rate_type,ETA1.end_time FROM base_customermasterfrieghtrates as ETA1 where ETA1.last_updated_at = ( Select MAX(ETA2.last_updated_at) from base_customermasterfrieghtrates ETA2 where ETA1.customer_id = ETA2.customer_id and ETA1.from_city_id = ETA2.from_city_id and ETA1.to_city_id = ETA2.to_city_id and ETA1.truck_type_id = ETA2.truck_type_id) and( ETA1.end_time = (Select CASE WHEN ETA2.end_time IS NOT NULL THEN  MAX(ETA2.end_time) ELSE ETA2.end_time  END from base_customermasterfrieghtrates ETA2 where ETA1.customer_id = ETA2.customer_id and ETA1.from_city_id = ETA2.from_city_id and ETA1.to_city_id = ETA2.to_city_id and ETA1.truck_type_id = ETA2.truck_type_id)   ;;
