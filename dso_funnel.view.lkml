view: dso_funnel {
  derived_table: {
    sql: Select m.sector_name, a.user_id ,d.name as Customer_name, sum(a.order_value) as total_business , c.payment ,f.billed_amt, e.daily_business_value  as daily_business_value from base_order as a left join ( Select b.customer_id , sum(b.amount) as payment from base_receivablepayment as b group by 1) c  on a.user_id = c.customer_id left join
